...
class ScrabbleScreen(Screen):
    ...
    def __init__(self, title, width, height):
        ...
        self.board = []
        self.rack = []
        # rack_surf and board_surf is the surface in which the rack and board outlines will be
        ...
        # current turn is the players currently placed tiles and where on the board the tiles were placed
        self.currentTurn = []
    ...
    '''
    This is no longer needed however it is the original make board function can be taken out
    ...
    '''
    # created the board 15 x 15 grid in a list of lists
    # requests the current board from the server
    def makeBoard(self):
        get_board = requests.get('http://localhost:3000/getdata').text
        #print(get_board)
        self.board = []
    ...
    def draw_pieces(self, selected_piece):
        ...
        # draws the pieces on the board used for placeing new tiles
        for y in range(0, 15):
            for x in range(0, 15):
        ...
        # used for drawing pieces on the rack
        for y in range(0, 7):
            piece = self.rack[y]
            if piece:
                color, type = piece
    ...
    # needs to be fixed and finished - Niall's job
    def endTurn(self):
        self.currentTurn = []
    ...
    def mainLoop(self):
        ...
        #Main Loop
        while True:
            # we always want to track the mouses postition if its on the rack or not
            piece, mouseX, mouseY = self.getMousePosRack()
            ...
            for e in pygame.event.get():
                ...
                if e.type == pygame.MOUSEBUTTONUP:
                    if drop_pos and selected_piece:
                        ...
                        selected_piece = None
                    drop_pos = None
            # colour and images drawed to screen
            self.screen.fill(BACKGROUND)
            self.screen.blit(self.board_surf, self.BOARD_POS)
            self.screen.blit(self.rack_surf, self.RACK_POS)
            # calls the functions to draw the outlines or the pieces if one is selected
            ...
            shuffleButton = self.drawButton(500, 550, "Shuffle", mouse)
            undoButton = self.drawButton(500, 600, "Undo", mouse)
            # gets the drop pos of the piece if one is selected
            drop_pos = self.draw_drag(selected_piece)
            # used to update the screen
            pygame.display.flip()
            # this is sort of like the fps counter can be taken out as we dont use fps in scrabble
            self.clock.tick(60)
