...
class ScrabbleScreen(Screen):
    ...
    #        return self
    def surf(self, dict, surf_spot, color=TILE_COLOR, tileMultiplyer=1):
        for y in range(0, len(dict)):
            for x in range(0, len(dict[y])):
                ...
                pygame.draw.rect(surf_spot, TILE_COLOR, rect)
    ...
    def mainLoop(self):
        ...
        #Load Assets
        #board surf
        self.surf(self.board, self.board_surf)
        #rack surf
        self.surf(self.rack, self.rack_surf, pygame.Color("blue"), 2)
        self.makeRack()
        selected_piece = None
        drop_pos = None
        #Main Loop
        while True:
            ...
            resetBoardButton = self.drawButton(100, 600, "Reset Board", mouse)
                ...
                for e in pygame.event.get():
                    ...
                    if e.type == pygame.MOUSEBUTTONDOWN:
                        ...
                        if resetBoardButton[0] <= mouse[0] <= resetBoardButton[0]+140 and resetBoardButton[1] <= mouse[1] < resetBoardButton[1]+40:
                            result = requests.get("{}/resetboard".format(self.serverip)).text
                            self.makeBoard()
                            #self.draw_pieces(None)
                            print('result')
